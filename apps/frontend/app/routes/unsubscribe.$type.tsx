...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  return [{ title: `Unsubscribe from ${typeLabel} - Analytodon` }];
};
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
    ...
    await usersApi.usersControllerUnsubscribe({
      type,
    ...
    return {
      success: `You have been successfully unsubscribed from ${typeLabel}.`,
    };
  } catch (error: unknown) {
    logger.error('Unsubscribe error:', error, { type, userId, email });
    ...
    if (apiError.response) {
      ...
      if (status === 404) {
        return new Response(JSON.stringify({ error: 'User not found or already unsubscribed.' }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (status === 400) {
        return new Response(JSON.stringify({ error: 'Invalid unsubscribe request.' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        });
      }
...
export default function Unsubscribe() {
  ...
  const isUnsubscribed = actionData && 'success' in actionData;
      ...
      <HeroSection>
        ...
        <HeroContent>
          ...
          <Typography variant="body1" sx={{ mt: 2, mb: 4, opacity: 0.9 }}>
            {isUnsubscribed ? "You've been unsubscribed successfully." : `Unsubscribe from ${typeLabel}.`}
          </Typography>
        </HeroContent>
      </HeroSection>
      ...
      <FormSection>
        <FormCard>
          <Typography variant="h5" component="h2" align="center" sx={{ mb: 3, fontWeight: 600 }}>
            {isUnsubscribed ? 'Unsubscribed' : 'Confirm Unsubscribe'}
          </Typography>
          ...
          {!hasRequiredParams ? (
            ...
            <>
              ...
              <Box sx={{ textAlign: 'center', mt: 2 }}>
                <Link
                  to={`/subscribe/${type}?u=${userId}&e=${email}`}
            ...
            <Box component={Form} method="post" noValidate>
              ...
              <Typography variant="body1" align="center" sx={{ mb: 3 }}>
                Are you sure you want to unsubscribe from <strong>{typeLabel}</strong>?
              </Typography>

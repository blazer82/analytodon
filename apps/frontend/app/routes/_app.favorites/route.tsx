...
export default function FavoritesPage() {
  ...
  const handleTimeframeChange = React.useCallback(
    (newTimeframe: Timeframe) => {
      ...
      if (accountId) {
        fetcher.load(buildLink(`/favorites?index&timeframe=${newTimeframe}`));
      }
    },
  ...
  const handleCSVDownload = React.useCallback(() => {
    if (accountId) {
      window.location.href = buildLink(`/favorites/csv?accountId=${accountId}&timeframe=${currentTimeframe}`);
    }

...
export const loader = withSessionHandling(async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    const [weeklyKPI, monthlyKPI, yearlyKPI, total, chartData, topTootsData] = await Promise.all([
      ...
      favoritesApi
        .favoritesControllerGetChartData({ accountId, timeframe: timeframeParam, dateFrom, dateTo })
        .catch(() => []),
      favoritesApi
        .favoritesControllerGetTopTootsByFavorites({ accountId, timeframe: timeframeParam, dateFrom, dateTo })
        .catch(() => []),
    ]);
...
export default function FavoritesPage() {
  ...
  const handleTimeframeChange = React.useCallback(
    (newTimeframe: Timeframe, dateRange?: DateRange) => {
      ...
      if (accountId) {
        let url = `/favorites?index&timeframe=${newTimeframe}`;
  ...
  const handleCSVDownload = React.useCallback(() => {
    if (accountId) {
      let url = `/favorites/csv?accountId=${accountId}&timeframe=${currentTimeframe}`;
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 4, lg: 3 }}>
          <TotalBoxPaper sx={{ p: 2, display: 'flex', flexDirection: 'column', height: 160 }}>
            {total ? (
              <TotalBox title={t('kpi.totalFavorites')} amount={total.amount} date={total.day} />
            ) : (
        ...
        <Grid size={{ xs: 12 }} sx={{ mt: 3 }}>
          <DataTablePaper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
            {hasTopTootsData && topTootsData && (
              <TopToots data={topTootsData} title={t('topPosts.title')} showBoosts={false} showReplies={false} />
            )}

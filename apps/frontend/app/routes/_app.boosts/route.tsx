...
export default function BoostsPage() {
  ...
  const handleTimeframeChange = React.useCallback(
    (newTimeframe: Timeframe) => {
      ...
      if (accountId) {
        ...
        fetcher.load(`/boosts?index&timeframe=${newTimeframe}${viewAsParam}`);
      }
    },
  ...
  const handleCSVDownload = React.useCallback(() => {
    if (accountId) {
      const viewAsParam = viewAs ? `&viewAs=${viewAs}` : '';
      window.location.href = `/boosts/csv?accountId=${accountId}&timeframe=${currentTimeframe}${viewAsParam}`;
    }

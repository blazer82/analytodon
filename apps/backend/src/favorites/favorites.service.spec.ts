...
describe('FavoritesService', () => {
  ...
  describe('getTopTootsByFavorites', () => {
    it('should return top toots mapped to FavoritedTootDto', async () => {
      mockAccountsService.findById.mockResolvedValue(mockAccount);
      const mockRawToots = [
        {
      ...
      const result = await service.getTopTootsByFavorites(mockAccountId, 'last7days', mockUser);
  ...
  describe('exportCsv', () => {
    ...
    it('should export chart data to CSV', async () => {
      ...
      expect(mockRes.setHeader).toHaveBeenCalledWith(
        'Content-Disposition',
        `attachment; filename=favorites-${mockAccountId}-last7days.csv`,
      );

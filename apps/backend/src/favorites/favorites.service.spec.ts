...
describe('FavoritesService', () => {
    ...
    kpiMethods.forEach((methodName) => {
      describe(methodName, () => {
        it('should return KPI data successfully', async () => {
          ...
          expect(callArgs[4]).toBe('favouritesCount');
  ...
  describe('getTotalSnapshot', () => {
    it('should return total snapshot if data exists', async () => {
      const mockStatEntry = { favouritesCount: 100, day: new Date() } as DailyTootStatsEntity;
      ...
      expect(result).toEqual({ amount: mockStatEntry.favouritesCount, day: mockStatEntry.day });
    });
  ...
  describe('getChartData', () => {
    it('should return chart data correctly', async () => {
      ...
      const mockStats = [
        { day: new Date('2023-01-09T00:00:00.000Z'), favouritesCount: 5 },
        { day: new Date('2023-01-10T00:00:00.000Z'), favouritesCount: 15 },
        { day: new Date('2023-01-11T00:00:00.000Z'), favouritesCount: 20 },
        { day: new Date('2023-01-12T00:00:00.000Z'), favouritesCount: 20 },
  ...
  describe('getTopTootsByFavorites', () => {
      ...
      const mockRankedTootEntities: (TootEntity & { rank: number })[] = [
        {
          ...
          visibility: 'public',
          createdAt: new Date(),
          rank: 50, // Example rank based on favorites
        },
      ];
      mockTootsService.getTopToots.mockResolvedValue(mockRankedTootEntities);

      const result = await service.getTopTootsByFavorites(mockAccount, 'last7days');
  ...
  describe('exportCsv', () => {
    ...
    it('should export daily stats as CSV with New Favorites and Total Favorites columns', async () => {
      ...
      expect(mockRes.setHeader).toHaveBeenCalledWith(
        'Content-Disposition',
        `attachment; filename=favorites-${mockAccount.id}-last7days.csv`,
      );

...
import { FavoritedTootDto } from '../../src/favorites/dto/favorited-toot.dto';
...
describe('FavoritesController (e2e)', () => {
  ...
  const testUserData = {
    email: 'favorites-e2e-user@example.com',
  ...
  beforeAll(async () => {
    ...
    testAccount.username = 'testuser_favorites';
    testAccount.accountName = '@testuser_favorites@mastodon.test';
    testAccount.accountURL = 'https://mastodon.test/@testuser_favorites';
    ...
    const statsData = [
      // Current data points
      { account: testAccount, day: todaySeed, boostsCount: 20, favouritesCount: 50, repliesCount: 4 },
      { account: testAccount, day: yesterdaySeed, boostsCount: 15, favouritesCount: 40, repliesCount: 3 },
      { account: testAccount, day: twoDaysAgoSeed, boostsCount: 10, favouritesCount: 30, repliesCount: 2 },

      // Weekly data points
      { account: testAccount, day: eightDaysAgoSeed, boostsCount: 5, favouritesCount: 20, repliesCount: 1 },
      { account: testAccount, day: fifteenDaysAgoSeed, boostsCount: 0, favouritesCount: 10, repliesCount: 0 },

      // Monthly data points
      { account: testAccount, day: thirtyDaysAgoSeed, boostsCount: 0, favouritesCount: 5, repliesCount: 0 },
  ...
  beforeEach(async () => {
    // For favorites, we might not need to clear DailyTootStats per test if `beforeAll` sets it up.
  ...
  const baseFavoritesPath = (accountId: string) => `/accounts/${accountId}/favorites`;
  ...
  describe('KPI endpoints', () => {
    ...
    kpiEndpoints.forEach((endpoint) => {
      it(`GET ${baseFavoritesPath(':id')}/${endpoint.path} - should get ${endpoint.name} KPIs`, async () => {
        const response = await request(app.getHttpServer())
          .get(`${baseFavoritesPath(testAccount.id)}/${endpoint.path}`)
    ...
    it(`GET ${baseFavoritesPath(':id')}/kpi/total - should get total snapshot`, async () => {
      const response = await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/kpi/total`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);

      expect(response.body).toBeInstanceOf(Object);
      expect(response.body).toHaveProperty('amount');
      expect(response.body.amount).toBe(50); // Based on seeded data
      expect(response.body).toHaveProperty('day');
    });

    it('should return 401 for KPI endpoints if not authenticated', async () => {
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/kpi/weekly`)
    ...
    it('should return 404 for KPI endpoints if account not found', async () => {
      ...
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(nonExistentAccountId)}/kpi/weekly`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.NOT_FOUND);
    });
  });

  describe(`GET ${baseFavoritesPath(':id')}/chart`, () => {
    it('should get chart data', async () => {
      const timeframe = 'last7days';
      const response = await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/chart?timeframe=${timeframe}`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);
    ...
    it('should return 400 if timeframe is missing for chart data', async () => {
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/chart`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.BAD_REQUEST);
    });
  });

  describe(`GET ${baseFavoritesPath(':id')}/top-toots`, () => {
    ...
    beforeAll(async () => {
      ...
      const toot1 = emFork.create(TootEntity, {
        ...
        url: 'https://mastodon.test/users/testuser_favorites/statuses/1',
        content: 'This is a top toot for favorites!',
        reblogsCount: 50,
        favouritesCount: 100,
        repliesCount: 10,
      ...
      const toot2 = emFork.create(TootEntity, {
        ...
        url: 'https://mastodon.test/users/testuser_favorites/statuses/2',
        content: 'Another favorited toot',
        reblogsCount: 150,
        favouritesCount: 50,
        repliesCount: 5,
    ...
    it('should get top toots by favorites', async () => {
      const timeframe = 'last7days';
      const response = await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/top-toots?timeframe=${timeframe}`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);

      expect(response.body).toBeInstanceOf(Array);
      expect(response.body.length).toBeGreaterThanOrEqual(1); // Expecting at least one toot
      const tootDto = response.body[0] as FavoritedTootDto;
      expect(tootDto).toHaveProperty('id');
      expect(tootDto).toHaveProperty('content');
      expect(tootDto).toHaveProperty('favouritesCount');
      expect(tootDto.favouritesCount).toBe(100); // Toot1 should be first
    });

    it('should return 400 if timeframe is missing for top toots', async () => {
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/top-toots`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.BAD_REQUEST);
    });
  });

  describe(`GET ${baseFavoritesPath(':id')}/csv`, () => {
    it('should download favorites data as CSV', async () => {
      const timeframe = 'last7days';
      const response = await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/csv?timeframe=${timeframe}`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);
    ...
    it('should return 400 if timeframe is missing for CSV export', async () => {
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(testAccount.id)}/csv`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.BAD_REQUEST);
    });
  });
  ...
  describe('Account Setup Incomplete', () => {
    ...
    beforeAll(async () => {
      ...
      incompleteAccount = await accountsService.create(
        { ...createAccountDto, name: 'Incomplete Favorites Account', serverURL: 'https://incomplete.test' },
        testUser,
      );
    ...
    it('should return 404 for KPI if account setup is not complete', async () => {
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(incompleteAccount.id)}/kpi/weekly`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.NOT_FOUND);
    });
    ...
    it('should return 404 for chart data if account setup is not complete', async () => {
      await request(app.getHttpServer())
        .get(`${baseFavoritesPath(incompleteAccount.id)}/chart?timeframe=last7days`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.NOT_FOUND);
    });
  });

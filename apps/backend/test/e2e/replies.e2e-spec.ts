...
import { RepliedTootDto } from '../../src/replies/dto/replied-toot.dto';
...
describe('RepliesController (e2e)', () => {
  ...
  const testUserData = {
    email: 'replies-e2e-user@example.com',
  ...
  beforeAll(async () => {
    ...
    testAccount.username = 'testuser_replies';
    testAccount.accountName = '@testuser_replies@mastodon.test';
    testAccount.accountURL = 'https://mastodon.test/@testuser_replies';
    ...
    const statsData = [
      // Current data points
      { account: testAccount, day: todaySeed, boostsCount: 20, favouritesCount: 10, repliesCount: 20 },
      { account: testAccount, day: yesterdaySeed, boostsCount: 15, favouritesCount: 8, repliesCount: 15 },
      { account: testAccount, day: twoDaysAgoSeed, boostsCount: 10, favouritesCount: 5, repliesCount: 10 },

      // Weekly data points
      { account: testAccount, day: eightDaysAgoSeed, boostsCount: 5, favouritesCount: 2, repliesCount: 5 },
  ...
  beforeEach(async () => {
    // For replies, we might not need to clear DailyTootStats per test if `beforeAll` sets it up.
  ...
  const baseRepliesPath = (accountId: string) => `/accounts/${accountId}/replies`;
  ...
  describe('KPI endpoints', () => {
    ...
    kpiEndpoints.forEach((endpoint) => {
      it(`GET ${baseRepliesPath(':id')}/${endpoint.path} - should get ${endpoint.name} KPIs`, async () => {
        const response = await request(app.getHttpServer())
          .get(`${baseRepliesPath(testAccount.id)}/${endpoint.path}`)
    ...
    it(`GET ${baseRepliesPath(':id')}/kpi/total - should get total snapshot`, async () => {
      const response = await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/kpi/total`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);
    ...
    it('should return 401 for KPI endpoints if not authenticated', async () => {
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/kpi/weekly`)
    ...
    it('should return 404 for KPI endpoints if account not found', async () => {
      ...
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(nonExistentAccountId)}/kpi/weekly`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.NOT_FOUND);
    });
  });

  describe(`GET ${baseRepliesPath(':id')}/chart`, () => {
    it('should get chart data', async () => {
      const timeframe = 'last7days';
      const response = await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/chart?timeframe=${timeframe}`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);
    ...
    it('should return 400 if timeframe is missing for chart data', async () => {
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/chart`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.BAD_REQUEST);
    });
  });

  describe(`GET ${baseRepliesPath(':id')}/top-toots`, () => {
    ...
    beforeAll(async () => {
      ...
      const toot1 = emFork.create(TootEntity, {
        ...
        url: 'https://mastodon.test/users/testuser_replies/statuses/1',
        content: 'This is a top toot for replies!',
        reblogsCount: 50,
        favouritesCount: 20,
        repliesCount: 100,
      ...
      const toot2 = emFork.create(TootEntity, {
        ...
        url: 'https://mastodon.test/users/testuser_replies/statuses/2',
        content: 'Another replied toot',
        reblogsCount: 20,
        favouritesCount: 10,
        repliesCount: 150,
    ...
    it('should get top toots by replies', async () => {
      const timeframe = 'last7days';
      const response = await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/top-toots?timeframe=${timeframe}`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);

      expect(response.body).toBeInstanceOf(Array);
      expect(response.body.length).toBeGreaterThanOrEqual(1); // Expecting at least one toot
      const tootDto = response.body[0] as RepliedTootDto;
      expect(tootDto).toHaveProperty('id');
      expect(tootDto).toHaveProperty('content');
      expect(tootDto).toHaveProperty('repliesCount');
      expect(tootDto.repliesCount).toBe(150); // Toot2 should be first
    });

    it('should return 400 if timeframe is missing for top toots', async () => {
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/top-toots`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.BAD_REQUEST);
    });
  });

  describe(`GET ${baseRepliesPath(':id')}/csv`, () => {
    it('should download replies data as CSV', async () => {
      const timeframe = 'last7days';
      const response = await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/csv?timeframe=${timeframe}`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.OK);
    ...
    it('should return 400 if timeframe is missing for CSV export', async () => {
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(testAccount.id)}/csv`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.BAD_REQUEST);
    });
  });
  ...
  describe('Account Setup Incomplete', () => {
    ...
    beforeAll(async () => {
      ...
      incompleteAccount = await accountsService.create(
        { ...createAccountDto, name: 'Incomplete Replies Account', serverURL: 'https://incomplete.test' },
        testUser,
      );
    ...
    it('should return 404 for KPI if account setup is not complete', async () => {
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(incompleteAccount.id)}/kpi/weekly`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.NOT_FOUND);
    });
    ...
    it('should return 404 for chart data if account setup is not complete', async () => {
      await request(app.getHttpServer())
        .get(`${baseRepliesPath(incompleteAccount.id)}/chart?timeframe=last7days`)
        .set('Authorization', `Bearer ${testUserAccessToken}`)
        .expect(HttpStatus.NOT_FOUND);
    });
  });
